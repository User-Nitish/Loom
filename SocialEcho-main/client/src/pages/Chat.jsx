...
import { Send, User, MessageCircle, Info, Hash, Zap, Search, MoreVertical, Paperclip, Smile } from "lucide-react";
...
const Chat = () => {
  ...
  // Handle auto-selection from navigation state or query params
  useEffect(() => {
    ...
    if (recipientId) {
      ...
      if (existingConv) {
        ...
        // Create a temporary "virtual" conversation for UI purposes
  ...
  return (
    <div className="flex h-[calc(100vh-200px)] min-h-[500px] bg-white/[0.02] backdrop-blur-3xl border border-white/5 rounded-[40px] overflow-hidden shadow-[0_50px_100px_rgba(0,0,0,0.8)] relative">
      ...
      <div className="w-80 border-r border-white/5 flex flex-col relative z-10 bg-black/20 backdrop-blur-xl">
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden bg-black/10">
        {selectedConversation ? (
          <div className="flex flex-col h-full">
            {/* Header */}
            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.02] relative z-10 backdrop-blur-2xl">
              <div className="flex items-center gap-4">
                <div className="relative">
                  <img
                    src={selectedConversation.participants.find(p => p._id !== user._id)?.avatar || "https://raw.githubusercontent.com/nz-m/public-files/main/dp.jpg"}
                    className="w-12 h-12 rounded-2xl object-cover border-2 border-white/10 shadow-2xl"
                    alt=""
                  />
                  <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-green-500 border-4 border-black/80 shadow-xl" />
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-[9px] font-black text-white/20 uppercase tracking-[0.2em]">Active Now</span>
                  </div>
                </div>
              </div>
              <div className="flex gap-2">
                ...
                <button className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-white/30 hover:text-white hover:bg-white/10 transition-all border border-white/5 md:hidden lg:flex">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-6 relative z-10">
              <div className="flex justify-center mb-10">
                <span className="px-6 py-2 bg-white/5 rounded-full text-[8px] font-black text-white/15 uppercase tracking-[0.6em] border border-white/5 backdrop-blur-md">
              ...
              {selectedConversation.virtual ? (
                <div className="flex flex-col items-center justify-center h-[60%] opacity-10">
                  ...
                  <p className="text-xs font-black uppercase tracking-[0.8em]">Start Conversation</p>
                </div>
              ) : (
                      ...
                      <React.Fragment key={msg._id || index}>
                        {showDate && (
                          <div className="flex justify-center my-8">
                            <span className="text-[9px] font-black text-white/10 uppercase tracking-[0.4em] px-4 py-1 border border-white/5 rounded-lg">
                        ...
                        >
                          <div className={`group relative max-w-[80%] p-5 rounded-[28px] ${isMine
                            ? "bg-gradient-to-br from-v-cyan to-v-cyan/80 text-black rounded-tr-none shadow-[0_20px_40px_rgba(27,206,220,0.15)]"
                            : "bg-white/[0.05] border border-white/10 text-white rounded-tl-none backdrop-blur-xl shadow-2xl"
                            }`}>
                            <p className="text-[13px] font-medium leading-relaxed tracking-wide select-text">{msg.content}</p>
                            <div className={`flex items-center gap-3 mt-3 ${isMine ? "text-black/40" : "text-white/20"}`}>
                              <span className="text-[8px] font-black uppercase tracking-widest">
                                {format(new Date(msg.createdAt), "HH:mm")}
                              </span>
                              {isMine && <Zap size={10} className="text-black/20" />}
                            </div>
                          </div>
                        </motion.div>
                      </React.Fragment>
            ...
            <div className="p-6 relative z-10 mt-auto">
              <form onSubmit={handleSendMessage} className="max-w-4xl mx-auto">
                <div className="flex items-center gap-3 bg-white/[0.03] border border-white/10 rounded-[32px] p-2 focus-within:border-v-cyan/50 focus-within:bg-white/[0.08] focus-within:shadow-[0_0_30px_rgba(27,206,220,0.1)] transition-all group backdrop-blur-xl">
                  ...
                  <input
                    ...
                    className="bg-transparent border-none outline-none flex-1 text-white text-[13px] font-bold px-4 py-3 placeholder:text-white/10 uppercase tracking-widest"
                    value={newMessage}
                  ...
                  <button
                    ...
                    className="w-12 h-12 bg-v-cyan text-v-ink rounded-[24px] flex items-center justify-center transition-all hover:scale-110 active:scale-90 disabled:opacity-30 disabled:grayscale disabled:scale-100 shadow-2xl shadow-v-cyan/20 group-focus-within:shadow-v-cyan/40"
                  >
          ...
          <div className="flex-1 flex flex-col items-center justify-center relative z-10 p-20 text-center">
            <motion.div 
              initial={{ scale: 0.8, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              className="w-40 h-40 bg-v-cyan/5 rounded-[50px] border border-v-cyan/10 flex items-center justify-center mb-12 relative group"
            >
              <div className="absolute inset-0 bg-v-cyan/20 blur-[100px] animate-pulse rounded-full opacity-50" />
              <MessageCircle size={80} className="text-v-cyan relative z-10 group-hover:scale-110 transition-transform duration-700" />
            </motion.div>
            <h3 className="text-3xl font-black text-white uppercase tracking-[0.6em] mb-6">Select a Chat</h3>
            <p className="text-[10px] text-white/20 font-bold max-w-xs leading-loose uppercase tracking-[0.4em]">
